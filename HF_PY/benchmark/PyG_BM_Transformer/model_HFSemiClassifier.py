...
class PointNetHF(nn.Module):
    """
    PointNet-style set encoder for hadron point cloud + electron feature classifier.

    输入:
      ele_feat: (B, ele_input_dim)
      had_feat: (B, N, had_input_dim)
      had_mask: (B, N) bool/0-1, True=valid, False=padding
    ...
    def __init__(
        ...
        n_classes: int = 2,
        use_ele_in_point_encoder: bool = True,  # 把 ele_feat 拼到每个 hadron 点特征里
        use_ele_feat: bool = True,              # 分类头是否额外拼 ele_feat
        pooling: str = "max",                   # "max" / "mean" / "sum"
    ):
        super().__init__()
        self.had_input_dim = had_input_dim
        self.ele_input_dim = ele_input_dim
        self.point_embed_dim = point_embed_dim
        self.use_ele_in_point_encoder = use_ele_in_point_encoder
        self.use_ele_feat = use_ele_feat
        self.pooling = pooling

        per_point_in = had_input_dim + (ele_input_dim if use_ele_in_point_encoder else 0)

        # shared MLP: φ(x_i)
        self.point_encoder = build_mlp(
            input_dim=per_point_in,
            hidden_dims=list(point_hidden_dims),
            output_dim=point_embed_dim,
            activation=nn.ReLU,
            last_activation=True,
        )

        # classifier: ρ([pool(φ), ele])
        clf_in = point_embed_dim + (ele_input_dim if use_ele_feat else 0)
        self.classifier = build_mlp(
            input_dim=clf_in,
            hidden_dims=list(clf_hidden_dims),
            output_dim=n_classes,
    ...
    def _masked_pool(self, z: torch.Tensor, mask: torch.Tensor):
        """
        z: (B, N, D)
        mask: (B, N) bool
        return: (B, D)
        ...
        B, N, D = z.shape
        if N == 0:
            return z.new_zeros((B, D))

        if self.pooling == "max":
            # padding 置为 -inf，然后 max；全 padding 的行再置 0
            neg_inf = torch.finfo(z.dtype).min
            z2 = z.masked_fill(~mask.unsqueeze(-1), neg_inf)  # (B,N,D)
            out = z2.max(dim=1).values                        # (B,D)
            ...
            denom = mask.sum(dim=1).clamp(min=1).unsqueeze(-1).to(z.dtype)  # (B,1)
            out = (z * mask.unsqueeze(-1).to(z.dtype)).sum(dim=1) / denom
            return out

        elif self.pooling == "sum":
            out = (z * mask.unsqueeze(-1).to(z.dtype)).sum(dim=1)
            return out
    ...
    def forward(self, ele_feat, had_feat, had_mask):
        ...
        # 1) 组 per-point 输入： [had_feat, ele_feat]（可选）
        if self.use_ele_in_point_encoder:
            ele_expand = ele_feat.unsqueeze(1).expand(B, N, ele_feat.shape[-1])  # (B,N,ele_dim)
            per_point = torch.cat([had_feat, ele_expand], dim=-1)                # (B,N,F+ele)
        else:
            per_point = had_feat

        # 2) shared MLP 编码每个点
        z = self.point_encoder(per_point.reshape(B * N, -1)).reshape(B, N, -1)  # (B,N,D)

        # 3) 对称聚合（PointNet 核心）
        H = self._masked_pool(z, had_mask)  # (B,D)

        # 4) 分类
        if self.use_ele_feat:
            clf_in = torch.cat([ele_feat, H], dim=-1)
        else:
            clf_in = H
...
class SetTransformerHF(nn.Module):
    """
    Minimal 2-layer Set Transformer for HF semi-leptonic e classification.

    输入:
      ele_feat: (B, ele_input_dim)
      had_feat: (B, N, had_input_dim)
      had_mask: (B, N) bool (True=valid, False=padding)

    做法:
      - 用 ele_feat 生成一个 CLS token（(B,1,d_model)）
      - 把 CLS 拼到 hadron tokens 前面： (B,1+N,d_model)
      - 2-layer TransformerEncoder 做 self-attention
      - 取输出的 CLS 向量做分类
    ...
    def __init__(
        ...
        self.had_proj = nn.Linear(had_input_dim, d_model)
        self.ele_proj = nn.Linear(ele_input_dim, d_model)  # CLS from electron
    ...
    def forward(self, ele_feat, had_feat, had_mask):
        ...
        # hadron tokens
        had_tokens = self.had_proj(had_feat)  # (B,N,d)

        # CLS token from electron
        cls = self.ele_proj(ele_feat).unsqueeze(1)  # (B,1,d)

        # concat: [CLS, hadrons...]
        tokens = torch.cat([cls, had_tokens], dim=1)  # (B,1+N,d)

        # key padding mask: True means "this position is PAD and should be ignored"
        # your had_mask: True=valid, False=pad
        pad_mask = ~had_mask  # (B,N) True=pad
        cls_pad = torch.zeros((B, 1), dtype=torch.bool, device=pad_mask.device)  # CLS never pad
        key_padding_mask = torch.cat([cls_pad, pad_mask], dim=1)  # (B,1+N)

        out = self.encoder(tokens, src_key_padding_mask=key_padding_mask)  # (B,1+N,d)
        H = out[:, 0, :]  # CLS
