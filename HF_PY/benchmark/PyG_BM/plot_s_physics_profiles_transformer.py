...
#       width/topology:
#           std_abs_dphi, mean_abs_deta, std_deta
#       hardness/activity:
...
def plot_profile(
    ...
    plt.plot(
        ...
        linewidth=1.8,
        markersize=4,
        label=r"mean score $\langle s\rangle$",
    )

    if band == "stderr":
        lo = prof["mean_s"] - prof["err_mean_s"]
        hi = prof["mean_s"] + prof["err_mean_s"]
        band_label = "stat. error on mean"
    elif band == "std":
        lo = prof["mean_s"] - prof["std_s"]
        hi = prof["mean_s"] + prof["std_s"]
        band_label = r"$\pm 1\sigma_s$"
    else:
        lo = prof["q16_s"]
        hi = prof["q84_s"]
        band_label = "16--84% score interval"
...
def selected_observables() -> List[Tuple[str, str, str]]:
    ...
    return [
        ...
        ("std_deta", r"std($\Delta\eta$)", "width_topology"),
...
def parse_args():
    ...
    p.add_argument(
        ...
        help=(
            "Band style: "
            "stderr = error on mean; "
            "quantile = 16--84% event interval; "
            "std = mean +/- event std."
        ),
    )
