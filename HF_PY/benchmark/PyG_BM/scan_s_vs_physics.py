...
# - Makes profile plots: <s> vs x (with std band), D/B separated
#
# Example:
...
def compute_observables(batch: Dict[str, torch.Tensor], use_log_pt: bool) -> Dict[str, np.ndarray]:
    ...
    # hadron: per-sample arrays -> compute stats
    ...
    for i in range(B):
        ...
        q_i = had_q[i, vi]
        same_sign_frac[i] = float(np.mean((q_i * e_q[i]) > 0.0))  # same sign with electron

    # some “shape-ish” ratios
    pt_conc = np.divide(lead_pt, np.maximum(sum_had_pt, 1e-12))  # leading pt fraction
...
def profile_plot(x: np.ndarray, s: np.ndarray, y: np.ndarray, var_name: str, out_png: str,
    ...
    """
    Make profile: <s> vs x, with std band. D/B separated.
    """
